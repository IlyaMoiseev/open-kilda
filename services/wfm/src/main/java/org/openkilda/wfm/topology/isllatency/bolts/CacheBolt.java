...
import org.openkilda.messaging.info.event.IslStatusUpdate;
...
public class CacheBolt extends AbstractBolt {
    ...
    private void handleGetDataFromCache(Tuple tuple) throws PipelineException {
        ...
        if (cache.containsKey(source)) {
            CacheEndpoint cachedData = cache.get(source);
            emitCachedData(tuple, data, cachedData, timestamp);
        } else {
            updateCacheAndEmit(tuple, data, source, timestamp);
        }
    }
    ...
    private void updateCacheAndEmit(Tuple tuple, IslRoundTripLatency data, CacheEndpoint source, long timestamp) {
    ...
    private void handleUpdateCache(Tuple tuple) {
        IslStatusUpdate data;
        try {
            data = pullValue(tuple, LATENCY_DATA_FIELD, IslStatusUpdate.class);

...
public class CacheBolt extends AbstractBolt {
    ...
    private Map<Endpoint, Endpoint> cache = new HashMap<>();
    ...
    private void handleGetDataFromCache(Tuple tuple) throws PipelineException {
        ...
        Endpoint source = Endpoint.of(data.getSrcSwitchId(), data.getSrcPortNo());
    ...
    private void emitCachedData(Tuple tuple, IslRoundTripLatency data, Endpoint destination, long timestamp) {
    ...
    private Endpoint updateCache(IslRoundTripLatency data, Endpoint source) {
        ...
        if (activeIsls.isEmpty()) {
            log.warn("There is no active ISLs with src endpoint {}_{}.", data.getSrcSwitchId(), data.getSrcPortNo());
            return null;
        }
    ...
    private void handleUpdateCache(Tuple tuple) {
        IslStatusUpdateNotification data;
        try {
            data = pullValue(tuple, LATENCY_DATA_FIELD, IslStatusUpdateNotification.class);
        } catch (PipelineException e) {
            unhandledInput(tuple);
            return;
        }

        if (data.getStatus() == IslStatus.MOVED) {
            Endpoint source = Endpoint.of(data.getSrcSwitchId(), data.getSrcPortNo());
            Endpoint destination = Endpoint.of(data.getDstSwitchId(), data.getDstPortNo());

...
public class VxlanPoolTest extends Neo4jBasedTest {
    ...
    public void vxlanIdPool() {
        ...
        int first = vxlanPool.allocate(flow, new PathId("path_1")).getVxlan().getVni();
        assertEquals(100, first);

        PathId path2 = new PathId("path_2");

        flow.setFlowId("flow_2");
        int second = vxlanPool.allocate(flow, path2).getVxlan().getVni();
        assertEquals(101, second);

        flow.setFlowId("flow_3");
        int third = vxlanPool.allocate(flow, new PathId("path_3")).getVxlan().getVni();
        assertEquals(102, third);

        vxlanPool.deallocate(path2);

        flow.setFlowId("flow_4");
        int fourth = vxlanPool.allocate(flow, new PathId("path_4")).getVxlan().getVni();
        assertEquals(101, fourth);

        flow.setFlowId("flow_5");
        int fifth = vxlanPool.allocate(flow, new PathId("path_5")).getVxlan().getVni();
    ...
    public void vxlanPoolFullTest() {
        for (int i = 100; i <= 111; i++) {
            ...
            assertTrue(vxlanPool.allocate(flow, new PathId(format("path_%d", i))).getVxlan().getVni() > 0);
        }
    }
}
